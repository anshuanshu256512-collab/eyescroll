...
public class EyeAccessibilityService extends AccessibilityService {
    public static final int GESTURE_SWIPE_DOWN=1,GESTURE_SWIPE_UP=2,GESTURE_TAP=3,GESTURE_DOUBLE_TAP=4,GESTURE_SUBSCRIBE=5;
    ...
    public static boolean isEnabled(Context ctx){
        ...
        return s!=null&&s.contains(ctx.getPackageName()+"/"+EyeAccessibilityService.class.getName());
    }
    ...
    public void performGesture(int type){
        ...
        int[]sz=getScreenSize();int W=sz[0],H=sz[1],cx=W/2,cy=H/2;
        switch(type){
            case GESTURE_SWIPE_DOWN:doSwipe(cx,(int)(H*.75f),cx,(int)(H*.25f),350);break;
            case GESTURE_SWIPE_UP:doSwipe(cx,(int)(H*.25f),cx,(int)(H*.75f),350);break;
    ...
    private void doSwipe(int x1,int y1,int x2,int y2,long d){
        ...
        dispatchGesture(new GestureDescription.Builder().addStroke(new GestureDescription.StrokeDescription(p,0,d)).build(),null,null);
    }
    ...
    private void doDoubleTap(int x,int y){
        if(Build.VERSION.SDK_INT<Build.VERSION_CODES.N)return;
        Path p1=new Path();p1.moveTo(x,y);Path p2=new Path();p2.moveTo(x,y);
        dispatchGesture(new GestureDescription.Builder().addStroke(new GestureDescription.StrokeDescription(p1,0,100)).addStroke(new GestureDescription.StrokeDescription(p2,250,100)).build(),null,null);
    }
